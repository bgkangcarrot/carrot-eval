export default async function handler(req, res) {
  ...
  // ── 모드 분기: 마스킹 vs 피드백 ──────────────────────────────────
  if (req.body._mode === 'feedback') {
    return handleFeedback(req, res, apiKey);
  }
  return handleMasking(req, res, apiKey);
}

// ── 마스킹 ──────────────────────────────────────────────────────────
async function handleMasking(req, res, apiKey) {
...
async function handleFeedback(req, res, apiKey) {
  const { agentName, satAvg, satScore, evalCnt, distSummary, opinions } = req.body;
...
- 이름: ${agentName}
- 평가건수: ${evalCnt}건
- 만족도 평균: ${satAvg?.toFixed(1)}점
- 만족도점수 (50점 환산): ${satScore?.toFixed(1)}점
- 평점 분포: ${distSummary}
